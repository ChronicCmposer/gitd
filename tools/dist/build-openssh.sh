...
case "${OPENSSH_MODE}" in
    with-openssl)
        ...
        APK_OPENSSL_DEPS="openssl-dev openssl-libs-static zlib-dev zlib-static"
        ;;
    without-openssl)
        ...
        # crypto is used. Deterministic but not the primary path.
...
cp "${DIST_DIR}/patches/openssh-${OPENSSH_VERSION}-auth-identity.patch" "${WORK}/src/"
...
patch -p1 < openssh-${OPENSSH_VERSION}-auth-identity.patch
./configure \
    --prefix=/usr/local \
    ...
    --disable-shared \
    ${OPENSSH_CONFIG_OPENSSL}
make -j"$(nproc)" CFLAGS="-O2 -static" LDFLAGS="-static"
...
chroot_run "${rootfs}" "${WORK}/build.sh" || die "openssh build failed"
...
file "${stage}/usr/local/bin/sshd" | grep -q "statically linked" \
